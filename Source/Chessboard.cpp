...
void Chessboard::winOrNot()
{
    ...
    if (3 == howManyChess(true) && 15 == sumOfHowMany(3, true))
    {
        ...
        stopTimer();
        AlertWindow::showMessageBox (AlertWindow::InfoIcon, TRANS("congrats!"), TRANS("You won!!"));
        removeAll(); 
        ++humanWins;          
    } 
    // computer win..
    else if (3 == howManyChess(false) && 15 == sumOfHowMany(3, false))
    {
        ...
        animator.cancelAllAnimations(true);
        stopTimer();
        AlertWindow::showMessageBox(AlertWindow::InfoIcon, TRANS("HiaHia..."), TRANS("I won!!"));
        removeAll();
        ++cptWins;   
    }    
}
...
void Chessboard::removeAll()
{
    ...
    for (int i =0; i < chess.size(); ++i)
        animator2.animateComponent(chess[i], Rectangle<int>(0, getHeight(), 0, 0), 0.45f, 800, true, 1.0, 0.0);
